<template>
  ...
  <!-- MODAL -->
  <!-- ================================================================================ -->
  <div class="modal fade" id="modalNewPipeLine" aria-hidden="true" tabindex="-1" ref="newPipeLineConfigModal">
    ...
    <div class="modal-dialog modal-dialog-centered mw-1000px">
      <div class="modal-content">
        <div class="modal-header">
          ...
          <div ref="Btn_Close" data-bs-dismiss="modal" class="btn btn-icon btn-sm btn-active-icon-primary">
        ...
        <div class="modal-body py-10 px-lg-17">
          <el-form ref="newPipelineForm" :model="state.form" label-position="top" :rules="state.inputRules">
            <el-row>
              <!-- 파이프라인 구분 -->
              <el-form-item class="me-3">
                <div style="display: flex; justify-content: start;">
                  ...
                  <el-form-item prop="newStageCd" class="ms-3 pe-7">
                    <el-input v-model="state.form.newStageCd" placeholder="추가할 스테이지 구분 입력" style="width: 200px;" />
                    <span class="svg-icon svg-icon-1" @click="onClickAddStageCd"
                ...
                <div>
                  <el-radio v-model="state.form.pipelineCd" v-for="(pipelineCd, idx) in state.stageCdList"
                    :key="pipelineCd.commonCd" :label="pipelineCd.commonCd" :value="pipelineCd.commonCd"
                    @change="onChangeValidatePipelineCd">
                    <label>{{ pipelineCd.commonCd }}</label>
                    <span 
                      v-if="pipelineCd.protectedYn == 'N'" 
                      class="svg-icon svg-icon-1"
                      @click="_deleteStageCdList(pipelineCd.commonCd)" 
            ...
            <el-form-item prop="pipelineName">
              ...
              <div>
                <el-input class="pipelineName-input" v-model="state.form.pipelineName"
                  :placeholder="$t('project.pipeLine.pipeLineNamePlaceHolder')" @input="onChangeValidate" />
                <el-button v-if="state.isDuplicateChecked == false" class="ml-10" type="primary"
                  :loading="state.isChecking" @click="_onDuplicate">
                  {{ $t("configuration.toolChain.btnCheckDuplicateCheck") }}
                </el-button>
                <el-button v-if="state.isDuplicateChecked == true" class="ml-10" type="success" icon="el-icon-success">
                  {{ $t("configuration.toolChain.btnCheckDuplicateCheck") }}
                </el-button>
              </div>
            </el-form-item>
            ...
            <el-form-item>
              ...
              <VAceEditor v-model:value="state.form.pipelineScript"
                :placeholder="$t('project.pipeLine.pipeLineContentsPlaceHolder')" style="height: 300px;" />
            </el-form-item>
          </el-form>
        </div>
        ...
        <div class="modal-footer flex-center">
          ...
          <button class="btn btn-light me-3" style="width:10em;" data-bs-dismiss="modal">
            {{ $t("common.cancel") }}
          </button>

          <button v-if="!state.isSaving" class="btn btn-primary" style="width:12em;" @click.prevent="_onSave()">
...
import { createJenkinsPipeLine, duplicateJenkinsPipelineName, getDefaultScript, getStageCdList, updateStageCdList, deleteStageCdList } from '@/api/jenkinsPipeline'
...
export default {
  ...
  setup(props, { emit }) {
    ...
    function onChangeValidatePipelineCd() {
      state.value.form.newStageCd = state.value.form.pipelineCd
      state.value.isDuplicateChecked = false
    }
    // =================================================================================
    // API
    ...
    async function onClickAddStageCd() {
      ...
      if(state.value.form.newStageCd.trim() == '') return;

      let success = false;
      
      (instance?.refs['newPipelineForm']).validateField('newStageCd',msg => {
        if(!msg) success=true;
      })
      ...
      try {
        const param = {
          ...
          commonCd: state.value.form.newStageCd.toUpperCase().trim(),
          codeName: state.value.form.newStageCd.toUpperCase().trim(),
    ...
    async function _deleteStageCdList(commonCd) {
      Swal.fire({
