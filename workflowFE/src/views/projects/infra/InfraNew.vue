        ...
        <div class="card-header" style="padding-left: 0px">
          <div class="card-title m-0">
            <h3 class="fw-bolder m-0">
              {{ $t("service.infra.title") }}
            </h3>
          </div>
        </div>
        <el-form ref="newItemForm" class="content-wrapper" label-position="top" label-width="100px"
          :model="state.editData" :rules="state.inputRules" autocomplete="on">

          <!-- 구분(providerCd) -->
          <el-form-item inline-message="true" prop="providerCd">
            <span class="field-label required">
              {{ $t("service.clusterConfig.clusterPrividerCd") }}
            </span>

            <div class="duplicate-name-wrapper d-flex">
              <el-select class="col-md-12" v-model="state.editData.providerCd" @change="onValidateSelect('providerCd')"
                @blur="onValidateSelect('providerCd')">
                <el-option v-for="item in state.providerList" :key="item.commonCd" :label="item.commonCd"
                  :value="item.commonCd" />
              </el-select>
            </div>
          </el-form-item>
          ...
          <el-form-item inline-message="true" prop="k8sName">
            <span class="field-label required">VM 명</span>
            <div class="duplicate-name-wrapper d-flex">
              <el-input v-model.trim="state.editData.k8sName" placeholder="가상서버 명을 입력하세요" @change="onChangeName" />
            ...
            <div class="duplicate-name-wrapper d-flex">
              <el-input
                ...
                @change="onChangeName"
              /> -->
              <el-button v-if="state.isDuplicateCheck == false" type="primary" @click="onDuplicateCheck">
                {{ $t("common.duplicateCheck") }}
              </el-button>

              <el-button v-if="state.isDuplicateCheck == true" type="success" icon="el-icon-success">
                {{ $t("common.duplicateCheck") }}
              </el-button>
            </div>
          </el-form-item>
          ...
          <el-form-item inline-message="true" prop="k8sDesc" class="desc-field-form">
            ...
            <el-input v-model="state.editData.k8sDesc" type="textarea" rows="3" />
          </el-form-item>
          ...
          <el-form-item inline-message="true" prop="argocdUrl">
            <div style="display: flex; justify-content: space-between;">
              <div style="width: 48%">
                ...
                <div class="duplicate-name-wrapper d-flex">
                  <el-select class="col-md-12" v-model="state.editData.spec">
                    <el-option v-for="item in state.testSpecList" :key="item.name" :label="item.name"
                      :value="item.name" />
                  </el-select>
                  <!-- <el-input v-model="state.editData.argocdUrl" :placeholder="$t('service.clusterConfig.msgArgocdUrl')"
              ...
              <div style="width: 48%">
                ...
                <div class="duplicate-name-wrapper d-flex">
                  <el-select class="col-md-12" v-model="state.editData.image">
                    <el-option v-for="item in state.testImageList" :key="item.name" :label="item.name"
                      :value="item.name" />
                  </el-select>
                  <!-- <el-input v-model="state.editData.argocdUrl" :placeholder="$t('service.clusterConfig.msgArgocdUrl')"
                @change="onChangeArgoCd" /> -->
          ...
          <el-form-item >
            <div style="display: flex; justify-content: space-between;">
              <div style="width: 48%">
                ...
                <div class="duplicate-name-wrapper d-flex">
                  <el-input v-model="state.editData.cpu" type="number" />
                </div>
              </div>
              <div style="width: 48%">
                <span class="field-label required">
                ...
                <div class="duplicate-name-wrapper d-flex">
                  <el-input v-model="state.editData.momery" type="number" />
                </div>
              </div>
            </div>
          </el-form-item>
          ...
          <el-form-item inline-message="true" prop="argocdUrl">
            <div style="display: flex; justify-content: space-between;">
              <div style="width: 48%">
                ...
                <div class="duplicate-name-wrapper d-flex">
                  <el-select class="col-md-12" v-model="state.editData.csp">
                    <el-option v-for="item in state.testCspList" :key="item.name" :label="item.name"
                      :value="item.name" />
                  </el-select>
                  <!-- <el-input v-model="state.editData.argocdUrl" :placeholder="$t('service.clusterConfig.msgArgocdUrl')"
                @change="onChangeArgoCd" /> -->
              ...
              <div style="width: 48%">
                ...
                <div class="duplicate-name-wrapper d-flex">
                  <el-select class="col-md-12" v-model="state.editData.region">
                    <el-option v-for="item in state.testRegionList" :key="item.name" :label="item.name"
                      :value="item.name" />
                  </el-select>
                  <!-- <el-input v-model="state.editData.argocdUrl" :placeholder="$t('service.clusterConfig.msgArgocdUrl')"
                @change="onChangeArgoCd" /> -->
          ...
          <div class="submit-wrapper">
            <el-button class="btn btn-light" size="medium" @click="cancelForm()">
              {{ $t("common.cancel") }}
            </el-button>

            <el-button class="btn btn-primary" size="medium" @click="submitForm()">
              {{ $t("common.save") }}
            </el-button>
          </div>
        </el-form>
...
import { duplicateCheck, createClusterConfig, connectionCheck } from "@/api/kubernetesConfig";
...
export default {
  ...
  setup(props) {
    ...
    const onValidateSelect = (prop) => {
        (instance?.refs.newItemForm as any).validateField(prop);
    };
    /* ============================================================================================================= */
    // 데이터
    /* ============================================================================================================= */
    ...
    const state = ref({
      ...
      editData: {
        ...
        // serviceGroupId: route.params.serviceGroupId,
        providerCd: '',
        // stageCd: '',
        k8sName: '',
        content: '',
      ...
      inputRules: {
        ...
        k8sName: [
          ...
          // //   required: true,
          // //   trigger: "blur",
          // //   validator: _validateAlphanumericUnderbarHyphen,
          // // },
        ],
        content: [
          // {
    ...
    function initData() {
      store.dispatch(clusterConfigActions.NAMESPACE + "/" + clusterConfigActions.CREATE_CLUSTER_CONFIG_INIT_DATA)
    }
    ...
    function submitForm() {
      ...
      if (state.value.isConnectionCheck == false) {
        ...
        // console.log(state.value.argocdPassword)
        return false;
      }
        ...
        if (valid) {
          (async () => {
            try {
              ...
              const response = await createClusterConfig(requestParam) as any;
          ...
          <el-form-item inline-message="true" prop="providerCd">
            ...
            <div class="duplicate-name-wrapper d-flex">
              <el-select 
                ...
                @change="onValidateSelect('providerCd')"
                @blur="onValidateSelect('providerCd')"
              >
                <el-option 
