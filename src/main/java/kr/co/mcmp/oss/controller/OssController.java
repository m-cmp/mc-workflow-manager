...
public class OssController {
    ...
    @Operation(summary = "List OSS", description = "List all OSS" )
    @GetMapping("/list")
    public ResponseWrapper<List<OssDto>> getOssList() {
        return new ResponseWrapper<>(ossService.getAllOssList());
    }

    @Operation(summary = "List OSS", description = "List OSS by keyword" )
    ...
    @Operation(summary = "Get OSS detail", description = "Get OSS detail" )
    @GetMapping("/{ossIdx}")
    public ResponseWrapper<OssDto> detailOss(@PathVariable Long ossIdx) {
        return new ResponseWrapper<>(ossService.detailOss(ossIdx));
    }

    @Operation(summary = "Check duplicate OSS information (name, URL, username)", description = "true: duplicate / false: not duplicate")
